...
계정: 전 410계정을 양쪽에 붙이면 ~840열 x 7,914,798행 (메모리 ~50GB) 이라 비현실적 —
핵심 17계정 + 기간 메타만 붙인다. 다른 계정이 필요하면 ITEM_IDS 만 바꿔 재실행
(또는 `attach_financials(..., items=[...])` 로 부착 시점에 고른다).
...
ITEM_IDS = [28, 34, 10, 21, 400, 4051, 15,            # 매출·매출원가·매출총이익·영업이익·EBIT·EBITDA·순이익
            ...
            2021, 2006]                                # CapEx·영업현금흐름 (2006 = Cash from Operations; D&A 는 2160/2171)
